...
export function validateRoiRequest(input: unknown): AutomationRoiRequest {
  ...
  const timePerTaskMinutes = parseNonNegativeNumber(input.timePerTaskMinutes, "timePerTaskMinutes");

  if (!isPlainObject(input.taskFrequency)) {
    throw new ValidationError("taskFrequency is required and must be an object.");
  }

  const count = parseNonNegativeNumber(input.taskFrequency.count, "taskFrequency.count");
  ...
  const laborCostPerHour = parseNonNegativeNumber(input.laborCostPerHour, "laborCostPerHour");
  const automationSetupCostEstimate = parseOptionalNonNegativeNumber(input.automationSetupCostEstimate, "automationSetupCostEstimate");
  const automationMaintenanceCostMonthly = parseOptionalNonNegativeNumber(input.automationMaintenanceCostMonthly, "automationMaintenanceCostMonthly");
  const humanReviewTimeMinutes = parseOptionalNonNegativeNumber(input.humanReviewTimeMinutes, "humanReviewTimeMinutes");
  const expectedAutomationCoveragePercent = parseOptionalNonNegativeNumber(input.expectedAutomationCoveragePercent, "expectedAutomationCoveragePercent");

  if (expectedAutomationCoveragePercent !== undefined && expectedAutomationCoveragePercent > 100) {
...
export function calculateAutomationROI(input: AutomationRoiRequest): AutomationRoiResponse {
  ...
  switch (input.taskFrequency.period) {
    case "day":
      monthlyVolume = input.taskFrequency.count * 30;
      break;
    case "week":
      monthlyVolume = input.taskFrequency.count * 4.33;
      break;
    case "month":
  ...
  const estimatedAutomatedHours = (automatedTasksVolume * input.timePerTaskMinutes) / 60;
  ...
  const assumptions = [
    ...
    "Estimated automated hours represents the original manual hours covered by automation, before human-review overhead.",
  ...
  return {
    ...
    estimatedAutomatedHours: Number(estimatedAutomatedHours.toFixed(2)),
